...
def _design_group(p, groups, rho_1, rho_2):
    """
    rho_1: within group correlation
    rho_2: between group correlation
    """
    Sigma = np.ones((p, p)) * rho_2
    for g in np.unique(groups):
        idx = np.where(groups == g)[0]
        Sigma[np.ix_(idx, idx)] = rho_1
    Sigma += np.eye(p) * (1 - rho_1)
    return np.linalg.cholesky(Sigma)


def gaussian_group_instance(n=100,
                            p=200,
                            groups=np.arange(20).repeat(10),
                            active_groups=5,
                            within_group_sparsity=2,
                            sigma=1.,
                            rho_1=0.8,
                            rho_2=0.2,
                            signal=7,
                            ...
                            center=True,
                            seed=1):
    chol = _design_group(p, groups, rho_1, rho_2)
    rng = np.random.default_rng(seed)
    X = rng.standard_normal((n, p)).dot(chol.T)
    ...
    beta = np.zeros(p) 
    signal = np.atleast_1d(signal)
    active_group_idx = rng.choice(np.unique(groups), active_groups, replace=False)
    for g in active_group_idx:
        idx = np.where(groups == g)[0]
        idx = rng.choice(idx, within_group_sparsity, replace=False)
        if signal.shape == (1,):
            beta[idx] = signal[0] 
        else:
            beta[idx] = np.linspace(signal[0], signal[1], len(idx))
        if random_signs:
            beta[idx] *= (2 * rng.binomial(1, 0.5, size=(len(idx),)) - 1.)
    beta /= np.sqrt(n)
