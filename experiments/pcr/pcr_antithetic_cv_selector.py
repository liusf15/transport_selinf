...
class PCRAntitheticCVSelection(Selector):
    # Select logistic principal-component dimension by antithetic cross-validation.
    def __init__(self, X_PC, y, alpha_cv=0.1, K_cv=10):
        # X_PC includes ordered PC scores; y is binary; alpha_cv controls the
        # train/test perturbation split and K_cv is the antithetic fold count.
        self.X_PC = X_PC
        self.y = y
        self.n = X_PC.shape[0]
        self.p = X_PC.shape[1]
        ...
        # Select a prefix of PCs, then refit its logistic target from the score.
        ...
        # Construct nuisance-score contrasts held fixed during conditional sampling.
    ...
    def estimate_score_cov(self):
        # Estimate the full score covariance using a ten-PC pilot logistic model.
    ...
    def logistic_regression_from_score(self, X, score):
        # Recover coefficients whose logistic score matches score for design X.
        l2_penalty = 1e-3
        d = X.shape[1]
    ...
    def antithetic_cv(self, X, s, sqrtH, alpha, K, rng):
        # X/s specify a candidate model; sqrtH scales perturbations; alpha and K
        # control the antithetic split; rng supplies reproducible Gaussian draws.
        d = s.shape[0]
        ...
        # Average held-out logistic risk across negatively correlated folds.
        err = 0
        for k in range(K):
            wk = sqrtH[:d, :d] @ W[k]
    ...
    def select(self, score):
        # Choose the lowest-risk prefix among the first five principal components.
    ...
    def _resample(self, rng, beta_null):
        # Simulate binary outcomes at beta_null, restore the conditioned nuisance
        # contrast, and accept only if the selected prefix is unchanged.
    ...
    def naive_inference(self, sig_level=0.05):
        # Return classical selected-model intervals, coefficient p-values, and LRT.
