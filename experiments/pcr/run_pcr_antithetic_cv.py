...
def run(seed, rho, n_train, n_val, n_fold=5):
    ...
    X_PC = preprocess_pipeline.fit_transform(X)
    X_PC /= np.linalg.norm(X_PC, axis=0, keepdims=True)
    X_PC_with_intercept = sm.add_constant(X_PC) 
    
    selector = PCRAntitheticCVSelection(X_PC_with_intercept, y, alpha_cv=0.1, K_cv=10)
    d = selector.d
    print('selected', d, 'PCs')
    if d == 1:
...
if __name__ == "__main__":
    ...
    if results is not None:
        ...
        prefix = f'pcr_antithetic_rho_{args.rho}_train_{args.n_train}_val_{args.n_val}'
