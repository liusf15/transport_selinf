...
def inference(logdensity_fn, beta_hat, Sigma, sig_level=0.05, compute_ci=False):
    d = len(beta_hat)
    pvalues = np.zeros(d)
    cis = np.zeros((d, 2))
    for j in range(d):
...
def run(seed, n_train, resample_scale, max_iter):
    ...
    naive_ci = pd.DataFrame({0: lower, 1: upper}, index=[f'x{i}' for i in range(1, d+1)])
    naive_pvalues = 2 * norm.cdf(-np.abs(beta_hat) / beta_sd)
    ...
    cis_all = {'naive': naive_ci, 'nf': nf_ci}
    print(cis_all)
    pvalues_all = {'naive': naive_pvalues, 'nf': nf_pvalue}
    coverages_all = {}
    for key, ci in cis_all.items():
        coverages_all[key] = (ci[0] <= beta_target) & (beta_target <= ci[1])
    coverages_all = pd.DataFrame(coverages_all)
    print(coverages_all)
    pvalues_all = pd.DataFrame(pvalues_all, index=coverages_all.index)
    print(pvalues_all)

    return {'coverages': coverages_all, 'pvalues': pvalues_all, 'cis': cis_all}
...
if __name__ == "__main__":
    ...
    seed = args.seed
    results = run(seed, n_train=args.n_train, resample_scale=args.resample_scale, max_iter=args.max_iter)
    if results is not None:
        savepath = os.path.join(args.rootdir, args.date, 'poly')
        if args.mixed_sign:
            prefix = f'poly_{n}_{p}_mixsign_signal_{args.signal_fac}_train_{args.n_train}_scale_{args.resample_scale}_adapt'
        else:
            prefix = f'poly_{n}_{p}_signal_{args.signal_fac}_train_{args.n_train}_scale_{args.resample_scale}_adapt'
